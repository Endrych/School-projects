...
**/
	...
	if(*RootPtr == NULL){  // Bude se jednat o koren stromu
		*RootPtr = malloc(sizeof(struct tBTNode));
		if(*RootPtr == NULL){ // Kontrola mallocu
			return;
		}
	...
	else{ // Jinak musime najit patricny uzel 
		tBTNodePtr current = *RootPtr;
		while(current != NULL){
			if(current->Cont < Content){ 
				if(current->RPtr == NULL){ // Pokud vpravem podstrome neni uzel muzeme vytvorit 
					current->RPtr = malloc(sizeof(struct tBTNode));
					if(current->RPtr == NULL){ // Kontrola mallocu
						return;
					}
					current->RPtr->Cont = Content;
					current->RPtr->LPtr = NULL;
					current->RPtr->RPtr = NULL;
					return;
				}
				current = current->RPtr;
			}
			else if(current->Cont > Content){
				if(current->LPtr == NULL){  // Pokud vlevem podstrome neni uzel muzeme vytvorit 
					current->LPtr = malloc(sizeof(struct tBTNode));
					if(current->LPtr == NULL){ // Kontrola mallocu
						return;
					}
					current->LPtr->Cont = Content;
					current->LPtr->LPtr = NULL;
...
**/
	tBTNodePtr current = ptr;
	while(current != NULL){
		BTWorkOut(current);
		SPushP(Stack, current);
		current = current->LPtr;
	}
}
...
**/
	...
	if(RootPtr != NULL){
		...
		do{
			tBTNodePtr curr = STopPopP(&Stack);
			BTWorkOut(curr); // Pri navratu do uzlu zpracovani
...
**/
	if(RootPtr != NULL){
		...
		tBTNodePtr current = RootPtr;
		Leftmost_Postorder(current,&pStack,&bStack);
		while(!SEmptyP(&pStack)){
			current = STopPopP(&pStack);
			left = STopPopB(&bStack);
			if(left){ // Prvni zpracovani
				SPushB(&bStack,FALSE);
				SPushP(&pStack,current);
				Leftmost_Postorder(current->RPtr,&pStack,&bStack);
			}
			else{ // Druhe zpracovani
				BTWorkOut(current);
			}
		}
	}
...
**/
	...
	if(*RootPtr != NULL){
		tStackP pStack;
		tBTNodePtr current = *RootPtr; // Ukazatel na uzly pro pruchod cyklem
		tBTNodePtr rm = *RootPtr;  // Ukazatel na uzly pro uvolnovani pameti
		SInitP(&pStack);
		while(current != NULL || !SEmptyP(&pStack)){ 
			if(current == NULL){ // Pokud by byl ukazatel null je potreba vzit uzel ze zasobniku
				current = STopPopP(&pStack);
			}
			if(current->RPtr != NULL){ // Potreba si ulozit pravy uzel, aby jsme se k nemu dostali
				SPushP(&pStack,current->RPtr);
			}
			rm = current;
			current = current->LPtr;
			free(rm);
