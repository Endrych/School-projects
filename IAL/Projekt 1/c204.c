...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
  ...
  while(!stackEmpty(s)){
    stackTop(s, &c);
    stackPop(s);
    //Pokud se narazi na ( dojde k preruseni funkce
    if(c == '(')
      return;
    else{
        //Ulozeni aktualniho znaku do vystupniho pole
        postExpr[*postLen] = c;
        (*postLen)++;
    }  
  }
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen ) {
  char stackChar;
  //Pokud je zasobnik prazdny znak se ulozi do zasobniku
  if(stackEmpty(s)){
    stackPush(s,c);
    return;
  }
  stackTop(s,&stackChar);
  //Zkoumani priority operatoru
  if(stackEmpty(s) || stackChar == '(' || ((c == '*' || c == '/') && (stackChar == '+' || stackChar == '-')))
    stackPush(s,c);
  else{
    stackPop(s);
    postExpr[*postLen] = stackChar;
    (*postLen)++;
    //Rekurzivni zpracovani operatoru
...
char* infix2postfix (const char* infExpr) {
  ...
  res = malloc(MAX_LEN * sizeof(char));
  if(res == NULL){
    return NULL;
  }
  unsigned res_pos =  0, pos = 0;
  //Alokace zasobniku
  tStack *s = NULL;
  ...
  while((c = infExpr[pos])){
    if(c == '='){
      while(!(stackEmpty(s)))
			{
				stackTop(s, &(res[res_pos]));
				stackPop(s);
				res_pos++;
			}																
			res[res_pos] = '=';
      res_pos++;				
      //Na konec retezce se vlozi nulovy znak a ukonceni cyklu						
      res[res_pos] = '\0';
      break;
    }
      ...
      //Zpracovani zasobniku
      untilLeftPar(s, res, &res_pos);
    }
    else{
      //Zpracovani operatoru
      doOperation(s, c ,res, &res_pos);
    }
    pos++;
  }
  free(s);
