...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  assert(head != NULL);
  return (void *) (head->offset + gl_VertexID * head->stride + (unsigned long)head->buffer);
}
...
void gpu_runVertexPuller(
  ...
  {
	  if (puller->heads[i].enabled == NULL)
...
void gpu_runPrimitiveAssembly(
  ...
  VertexShaderInvocation invocation = baseVertexShaderInvocation;
  GPUVertexShaderInput shaderInput;
  GPUVertexPullerOutput pullOutput;
  for (int i = 0; i < nofPrimitiveVertices; i++)
  {
	  gpu_runVertexPuller(&pullOutput, puller, invocation);
	  shaderInput.attributes = &pullOutput;
	  shaderInput.gl_VertexID = gpu_computeGLVertexID(puller->indices, invocation);
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  assert(lines       != NULL);
  (void)lines;
  float triangle = 1. / ((vertices[1].data[1] - vertices[2].data[1])* (vertices[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0]) * (vertices[0].data[1] - vertices[2].data[1]));
  coords->data[0] = ((vertices[1].data[1] - vertices[2].data[1])* (pixelCenter->data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0])*(pixelCenter->data[1] - vertices[2].data[1])) * triangle;
  coords->data[1] = ((vertices[2].data[1] - vertices[0].data[1])* (pixelCenter->data[0] - vertices[2].data[0]) + (vertices[0].data[0] - vertices[2].data[0])*(pixelCenter->data[1] - vertices[2].data[1])) * triangle;
